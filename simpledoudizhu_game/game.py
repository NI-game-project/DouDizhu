...
from simpledoudizhu_game.player import SimpleDoudizhuPlayer
from simpledoudizhu_game.round import SimpleDoudizhuRound
from simpledoudizhu_game.judger import DoudizhuJudger
from simpledoudizhu_game.utils import cards2str, doudizhu_sort_card
from simpledoudizhu_game.utils_global import get_downstream_player_id, get_upstream_player_id
...
class SimpleDoudizhuGame(object):
    ...
    def init_game(self):
        ...
        # initialize players
        self.players = [SimpleDoudizhuPlayer(num, self.np_random)
                        for num in range(self.num_players)]

        # initialize round to deal cards and determine landlord
        self.round = SimpleDoudizhuRound(self.np_random)
        self.round.initiate(self.players)

        # initialize judger
        self.judger = DoudizhuJudger(self.players, self.np_random)
